...
: PlannerBase("collision_planner", options)
{
  ...
  col_obj_sync_timer_ = create_wall_timer(std::chrono::seconds(3), std::bind(&CollisionPlanner::col_obj_sync_cb, this), timer_cbg_);

  for (RobotArm arm : all_arms)
  {
    add_col_obj_cli_[arm] = create_client<AddCollisionObjects>(
      "/" + arm_to_str.at(arm) + "/add_collision_objects",
      rmw_qos_profile_services_default,
      cli_cbg_);

    remove_col_obj_cli_[arm] = create_client<RemoveCollisionObjects>(
      "/" + arm_to_str.at(arm) + "/remove_collision_objects",
      rmw_qos_profile_services_default,
      cli_cbg_);

    apply_attach_col_obj_cli_[arm] = create_client<ApplyAttachedCollisionObjects>(
      "/" + arm_to_str.at(arm) + "/apply_attached_collision_objects",
      rmw_qos_profile_services_default,
      cli_cbg_);

    move_col_obj_cli_[arm] = create_client<MoveCollisionObjects>(
      "/" + arm_to_str.at(arm) + "/move_collision_objects",
      rmw_qos_profile_services_default,
      cli_cbg_);

    get_col_obj_from_secne_cli_[arm] = create_client<GetCollisionObjectsFromScene>(
      "/" + arm_to_str.at(arm) + "/get_collision_objects_from_sence",
      rmw_qos_profile_services_default,
      cli_cbg_);
  }
...
void CollisionPlanner::init_cb(void)
{
  for (RobotArm arm : all_arms)
  {
    if (!cli_wait_for_srv<AddCollisionObjects>(add_col_obj_cli_[arm], ""))
      return;
    if (!cli_wait_for_srv<RemoveCollisionObjects>(remove_col_obj_cli_[arm], ""))
      return;
    if (!cli_wait_for_srv<ApplyAttachedCollisionObjects>(apply_attach_col_obj_cli_[arm], ""))
      return;
    if (!cli_wait_for_srv<MoveCollisionObjects>(move_col_obj_cli_[arm], ""))
      return;
    if (!cli_wait_for_srv<GetCollisionObjectsFromScene>(get_col_obj_from_secne_cli_[arm], ""))
      return;
  }
...
void CollisionPlanner::col_obj_sync_cb(void)
{
  ...
  CollisionObject obj;
  obj.frame_id = "right_tcp";
  obj.id = 100;
  obj.type = 1;
  obj.name = "target";
  obj.dimension = {1.0, 1.0, 1.0};
  obj.pose = cvt_g_to_pose(get_g(0, 0, 1, 0, 0, 0));
  obj.visible = true;
  obj.dynamic = false;

  auto msg = std::make_shared<moveit_msgs::msg::CollisionObject>();
  if (cvt_moveit_col_obj(msg, obj))
  {
    RCLCPP_INFO(get_logger(), "cvt_moveit_col_obj");
  }
  if (add_col_obj(RobotArm::RIGHT, std::move(msg)))
  {
    RCLCPP_INFO(get_logger(), "add_col_obj");
  }

  RCLCPP_INFO(get_logger(), "!!!!!!!!!!!!!!!!!!!!!");
  col_obj_sync_timer_->cancel();
}
...
bool CollisionPlanner::get_col_obj_from_scene(RobotArm arm, std::vector<std::string>& object_ids)
{
  ...
  if (!send_sync_req<GetCollisionObjectsFromScene>(get_col_obj_from_secne_cli_[arm], std::move(request), response, __FUNCTION__))
  {
...
bool CollisionPlanner::add_col_obj(RobotArm arm, const std::vector<std::string>& existing_object_ids)
{
  ...
  if (!(send_sync_req<AddCollisionObjects>(add_col_obj_cli_[arm], request, response, __FUNCTION__) && response))
  {
...
bool CollisionPlanner::add_col_obj(RobotArm arm, const moveit_msgs::msg::CollisionObject::SharedPtr object)
{
  std::vector<std::string> existing_object_ids{};

  if (!get_col_obj_from_scene(arm, existing_object_ids))
  {
    RCLCPP_INFO(get_logger(), "Failed to get_col_obj_from_scene");
    return false;
  }
  ...
  auto request = std::make_shared<AddCollisionObjects::Request>();
  request->collision_objects.push_back(*object);

  AddCollisionObjects::Response::SharedPtr response;
  if (!send_sync_req<AddCollisionObjects>(add_col_obj_cli_[arm], request, response, __FUNCTION__))
  {
    RCLCPP_ERROR(get_logger(), "Sent AddCollisionObjects request failed");
    return false;
  }
  ...
  const CollisionObject object)
{
  ...
  if(object.mesh_file.empty()) 
  {
    ...
    RCLCPP_WARN(get_logger(), "Created a SolidPrimitive object: %d", object.id);
  } 
  else 
  {
    // load mesh
